...
import { IAuthResponse, IDataUser, IRegister } from "../../features/interfaces/auth"
...
export const UserProvider = ({ children }:IChildren) => {
  ...
  const loginUser = async(dataUser:IRegister):Promise<void> => {
    try {
      const { data: { accessToken, user: { id } } }:IAuthResponse = await api.post("/login", dataUser)
  ...
  const registerUser = async(dataUser:IDataUser):Promise<void> => {
    try {
      const { data: { accessToken, user: { id } } }:IAuthResponse = await api.post("/register", dataUser)
      localStorage.setItem("@rentalToken", accessToken)
      localStorage.setItem("@rentalId", JSON.stringify(id))
      navigate("/profile")
    } catch(err) {
